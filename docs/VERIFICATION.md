...
完整 P7.5尚未通过：correctness record采集时隐藏占用/可用显存为
`17,282/14,869 MiB`；2026-07-17再次采样为`17,102/15,049 MiB`。两者都不足以构建
约`17.4 GiB` peak的Qwen3-VL-8B engine，且外部 workload会污染 timing。GPU恢复后必须补 full-model
...
（`16.330 GiB`）和CUDA free memory，不加载权重。GPU短暂空闲时该命令曾PASS；隐藏
workload恢复后会正确FAIL，不能用短暂快照关闭KI-001。
...
- [ ] fresh环境完整8B demo、当前主线full regression：KI-001隐藏GPU workload阻塞。
- [ ] P7.5 full-engine/performance：同一blocker，不能作为P8完成项隐式跳过。

因此P8静态交付为PASS，阶段总状态为`PARTIAL`。只有最后两项动态门禁完成，外部用户
才能被宣称已在fresh环境复现核心8B结果。
...
并确认动态未验证项仍在`docs/KNOWN_ISSUES.md`和`docs/CLAIMS.md`中显式受限。
