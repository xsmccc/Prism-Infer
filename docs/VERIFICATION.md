...
该检查点边界：H1 BF16 batch1 与 batch4 已 formal PASS，P9-008 Verified；batch9–15 等仍
使用 sparse bucket，不能外推 token-exact。scaled-FP8 在下一检查点单独运行，不能由 BF16
结果预先判定。

### P9-D.2 H1 scaled-FP8 batch1/4 model-only Graph formal matrix（PASS，2026-07-20）

commit `d28e68a1a1d09918936d5dacfa5327b5e7be8381` 在同一 GPU UUID 上完成
`scaled_fp8_kv` 对 `scaled_fp8_kv_graph` 的两个 clean formal cells。每个 cell 都是
warmup2、每 mode 5 个 fresh processes、ABBA/BAAB、10,000 次 process bootstrap；220 个
blocks 的 payload `4,152,360,960 B` + scale `129,761,280 B` = `4,282,122,240 B`，
容量为 56,320 tokens。这里固定约 4 GiB pool 来比较 runtime；P9-C 的同 logical capacity
质量矩阵已另行证明 scaled-FP8 相对 BF16 的 pool 比例 `0.515625x` 和三数据集 formal
non-inferiority PASS。

正式 artifact：

- batch1 aggregate `data/p9_baseline/h1_scaled_fp8_b1_d28e68a.jsonl`，SHA256
  `d23d19d42a368c491874a24969c44603e5e50015f5c05b484c262ee6fefd9f61`；manifest SHA256
  `c778736b342357d0eee1eb9de873269653ebd00bd70a4855131f1092c699c6c0`；
- batch4 aggregate `data/p9_baseline/h1_scaled_fp8_b4_d28e68a.jsonl`，SHA256
  `1fc08774b5ea2359115501f642f970dbcaf4b849c186fbd30a7398fed80b367f`；manifest SHA256
  `12d8af92a67a5a98ba0ebe4288f73ae99f19bfe28811dabfabb932fa53f9a31b`。

两个 manifest 均为 `status=completed / formal_eligible=true`、15/15 comparability PASS、
token/text exact，合计 20/20 fresh children 前后均为 `1 MiB / 0%`。batch1 的 127 个 decode
steps 全为 `1→1`；batch4 为 `1→1:2 / 2→2:2 / 3→3:2 / 4→4:124`。

| scaled-FP8 指标 | Eager median | Graph median | 改善与 process-bootstrap 95% CI |
| --- | ---: | ---: | ---: |
| Batch1 decode step | 34.739 ms | 19.162 ms | 44.84% `[44.50%, 47.97%]` |
| Batch1 decode throughput | 28.752 tok/s | 52.176 tok/s | 81.47% `[80.49%, 92.31%]` |
| Batch1 end-to-end | 4898.21 ms | 2810.74 ms | 42.62% `[39.57%, 44.98%]` |
| Batch1 engine output throughput | 27.203 tok/s | 47.001 tok/s | 72.78% `[66.77%, 82.22%]` |
| Batch4 decode step | 35.669 ms | 19.998 ms | 43.93% `[43.65%, 45.17%]` |
| Batch4 decode throughput | 109.126 tok/s | 195.372 tok/s | 79.03% `[78.01%, 82.58%]` |
| Batch4 end-to-end | 6245.12 ms | 4200.98 ms | 32.73% `[30.60%, 34.93%]` |
| Batch4 engine output throughput | 91.741 tok/s | 145.025 tok/s | 58.08% `[54.45%, 62.02%]` |

Graph peak allocated 相对 eager 为 `+8.16 MiB`，reserved 为 `+24 MiB`；batch1 capture
范围 `984.813–1092.144 ms`，batch4 为 `998.023–1062.998 ms`。capture 不在 request
timing scope，但属于 startup/ownership 成本。

TTFT 不做正向汇总：batch4 的 engine 与 preprocessing-inclusive CI 都跨零；batch1
preprocessing-inclusive CI 也跨零，而 engine-only TTFT 从 `281.480` 增至 `290.818 ms`，
正式回退 3.32%，95% CI `[0.55%, 37.35%]`。raw Graph 值包含一个 `386.608 ms` 高值，
按冻结协议完整保留，不删除或重跑美化。该问题登记为 P9-009，进入 NSYS prefill/host
timeline 归因。

当前判定：BF16/scaled-FP8 × batch1/4 四个 model-only Graph formal cell 全部 correctness/
decode/E2E PASS；model-only Graph baseline 阶段完成。P9-D 下一步是同一 workload 的 NSYS
pipeline 分解，再决定 greedy full-step Graph 的 capture 边界；不能把当前 model-only 结果
写成 LM head、sampler 或 full-step 已被 capture。

### P9-D.3 NSYS host coverage 与 analyzer scalability 修复（candidate PASS，2026-07-20）

P9-009 的第一轮 clean diagnostic 使用 commit `5ef051d`、同一 RTX 5090 UUID、H1
scaled-FP8 batch1、output128、warmup2/repeat1，并以 CUDA Profiler API 只捕获额外 profile
iteration。eager/Graph 的 benchmark 与 semantic output hash 均为
`d181bac75f27d56c554f7b7bd847e844ab1f618b9d37b6fd0b7e7fff5cca6b83`，与 formal
matrix exact；两条 trajectory 都是 1 prefill + 127 decode，Graph 为 127 次 `1→1`。

第一轮 trace 已能确认：prefill 的 kernel count 完全相同（`4317/4317`），kernel busy 为
`198.453/198.491 ms`，说明该 observation 中没有额外 Graph prefill kernel 工作；decode
eager 每步 1,965 次显式 launch，Graph 为 1 次 graph launch + 9 次显式 launch，node-level
kernel busy 均约 17.94 ms。这里是受 profiler 扰动的归因数据，不替代 formal TTFT/TPOT。

但 trace audit 同时发现 CPU-only preprocess/scheduler region 不在 NVTX，且旧 analyzer 对
59 万事件、4,572 个 attention ranges 的 eager trace 超过 60 秒。修复内容：

- CPU-only region 在 active CUDA profiling session 中发射 NVTX，但不创建 CUDA Event；
- 每个 profile step 增加 `prism::engine.step` 父 range；
- analyzer read-only 打开 raw SQLite，一次加载 CUPTI activity，再按 start time 和
  `correlationId` 批量关联；engine phase summary 增加 `cpu_range_ms`；
- CLI quiet 写入且拒绝覆盖 artifact；分析函数拆分后 benchmark complexity PASS。

真实 H1 raw trace 上，新旧既有 summary 字段逐字节相同；完整 eager/Graph summary SHA256
分别为 `5a063ccd0f0c816e8057f8fa079b80f7c2fe9db57e6e234cda9711881d02acd8` 和
`1414dd5920b9ccdfb4b1cba7d0a970b119e7241ffee38fbbf7d8d931fc61e8be`。重构后 wall time
为 eager `5.28 s`、Graph `2.98 s`；被拒绝的 TEMP-index 候选仍超过 60 秒。

dirty single-image/output4 integration smoke 进一步确认 SQLite 中有：

```text
prism::engine.step                         4
prism::engine.scheduler.schedule           4
prism::engine.scheduler.postprocess        4
prism::preprocess.image_processor          1
prism::preprocess.mrope_positions          1
```

以 `engine.step` 分析得到 1 prefill + 3 decode；CPU-only targets 的 direct kernel count 和
GPU busy 都为 0。focused profile/analyzer/P7 summary 为 `11 passed`，ruff、complexity、
compileall 和 diff check PASS。完整测试集 JUnit 记录 `472 tests, 0 failures, 0 errors,
1 skipped, 303.502 s`；结束后 GPU 为 `1 MiB / 0%`。完整问题链见 P9-010。

当前边界：工具 candidate PASS，但 dirty smoke 不形成性能 claim。提交后必须重新采集 clean
