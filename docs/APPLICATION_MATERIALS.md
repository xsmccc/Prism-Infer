...
compaction，并用 Systems trace闭环优化热点。
...
KV tracing, and content-aware physical visual-KV compaction, backed by layered HF
correctness and systems-profiling gates.
...
- 实现 BF16/FP8 paged decode和KV compaction Triton kernels；32-case Qwen GQA
  kernel correctness通过 independent SDPA reference，并通过 tensorized slot mapping
...
1.34–1.40 倍，并继续从 Graph内 linear热点推进，而不是只挑有利数字。
...
kernel correctness和storage reduction不等于生成质量。FP8长输出发生早期分叉，没有
通过最终quality gate，所以保留为实现/失败研究，不进入默认路径。capacity数字也必须
同时说明quality FAIL。
