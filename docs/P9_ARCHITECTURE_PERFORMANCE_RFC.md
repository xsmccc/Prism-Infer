...
> 状态：P9-B PASS；P9-C SCALED FP8 FORMAL QUALITY PASS；EXTERNAL QUALITY
> FORMAL MATRIX COMPLETE / MIXED；FULL-PHYSICAL GATE A PENDING
> 日期：2026-07-19
...
- P9 入口时的 FP8 仅把 BF16 K/V 直接写入 FP8 tensor，等价于 scale=1；该
  unit-scale 路径的长输出质量 gate 已失败。P9-C 后新增的 scaled 路径不继承此结论。
...
Vision attention 使用独立的 startup policy：`sdpa` 或 `flash_attn`。包存在性只作为
capability gate，不能隐式选择 backend；默认 SDPA 保持 HF strict reference，
FlashAttention 作为独立质量/性能候选。artifact 必须记录该字段，禁止跨 vision backend
聚合。P9-004 记录了隐式选择在多图/视频 BF16 路径造成的 full-model 数值漂移。
...
Prism 内部 backend A/B 由`benchmarks/run_p9_process_matrix.py`执行上述合同。parent
必须保持标准库-only且不初始化 CUDA；每个 child 前后按物理 GPU UUID 检查 idle/release，
并对 environment、model、workload、sampling、KV layout 与非目标 backend 字段执行 exact
comparability gate。输出到非 ignored 仓库路径、覆盖既有 artifact 或中途 Git identity
变化都必须在形成 ratio 前失败。

## 7. KV Cache 与量化主线

### 7.1 为什么 unit-scale FP8 baseline 失败

P9 入口基线的 store kernel 直接把 BF16 K/V 写入 FP8 cache，paged decode 再直接
load 为 FP32。没有 quant/dequant scale，所以实际采用固定 scale=1。该路径在短输出
偶然 token exact，但长输出最终 quality gate 已失败；它现在作为独立 `fp8_kv`
rejected mode 保留。P9-C 新增的 `scaled_fp8_kv` 位于
`prism_infer/ops/kv_cache_store.py`，不能与该失败基线混称为“FP8”。
...
- 2026-07-19 已把 compile ownership 集中到显式 backend：fullgraph 只包含纯
  QKV projection、QK-Norm、M-RoPE。曾捕获 mutable KV store 的版本会错误处理
  monolithic cache 的非零 `storage_offset` view，并可能 illegal memory access；KV store
  与 paged decode 现固定留在 validated runtime 边界。
- 收缩后的真实 8B batch1/output32 exact，但 batch2/output32 仍在后段分叉，所以状态
  继续为 rejected benchmark candidate；它不是 P9-D full-step Graph/compile+Graph PASS。
...
完整 pipeline 边界、证据字段、correctness matrix、止损规则和教学顺序冻结在
[`P9_COMPILER_GRAPH_PLAYBOOK.md`](P9_COMPILER_GRAPH_PLAYBOOK.md)。P9-D 所谓“优化到
极致”以该合同验收，不以 graph node/kernel 数或 capture 覆盖率单独验收。
...
当前租约只分配一张 RTX 5090（GPU0）。管理员开放 NCU/NSYS 权限后，宿主机额外设备
会出现在 `nvidia-smi` 中；这只是可观测性，不是调度分配或计算授权。因此 TP2 当前为
**NOT RUN / UNVERIFIED**，P9-A 期间跨 GPU0–1 的 Prism 与隔离环境 collective 尝试均
作废，不能用于判断 GPU 可用性，也不能把失败归因于 Torch/CUDA/NCCL/SM120。

P9 单卡 headline 不等待 TP。后续所有 GPU 实验固定 `CUDA_VISIBLE_DEVICES=0`。只有
未来明确租用或获准使用至少两张卡，才恢复逐卡 allocation、最小 collective、TP2
correctness/memory/latency 门禁；未经单独批准不升级当前已完成 P8 回归的主环境。

## 12. P9 日程与止损点

| 日期 | 阶段 | 交付物 | Stop/go gate |
|---|---|---|---|
| 07-17–07-18 | P9-A 规范与正式基线 | 本 RFC、fairness manifest、page/NCU 与 TP 资源边界 | 协议与证据可审计 |
...
### 13.1 P9-B 出口与 P9-C 进入条件

- [x] `ModelConfig/CacheConfig/SchedulerConfig/ExecutionConfig/QuantizationConfig/
  ServingConfig` 组成 frozen `PrismConfig`；旧 flat kwargs 只保留一个兼容周期，未知
  字段和错误类型在 HF/CUDA 副作用前失败。
- [x] HF-derived runtime `Config` 不再原地回填 EOS/KV capacity；使用
  `with_eos()`、`with_cache_capacity()` 生成新对象，并通过 pickle 与真实 spawn。
- [x] 删除 `Sequence` 类级 page/ID state；page size 与 request ID 都是构造必填项，
  ID 由 engine-owned allocator 产生，TP pickle 保留 request/page/FSM identity。
- [x] scheduler 到 executor 使用 immutable `BatchPlan`；backend 执行边界使用不携带
  mutable `Sequence` 的 immutable `DeviceBatch`，并严格检查 phase/context/cardinality。
- [x] 显式退出断开 runner/backend ownership，runner exit 失败时仍清理；同一进程连续
  single/multi-image/video HF -> Prism 8B 门禁 PASS，不再保留模型/KV 级显存泄漏。
- [x] `eager/compile/cuda_graph/compile_graph` 为显式 startup selection；未实现的
  `compile_graph`、不安全组合与超出 capture contract 的 batch 在启动时失败。
- [x] 真实 Qwen3-VL-8B eager 与最小 CUDA Graph capture/replay 在已分配 GPU0 上 PASS，
  退出后 GPU0 释放；scaled FP8、NVFP4、W/A quantization 和新 kernel 未混入本阶段 diff。
- [x] TP2 明确记录为 NOT RUN / UNVERIFIED；此前跨未分配 GPU1 的尝试作废，不形成
  NCCL root-cause、硬件可用性或 TP 能力结论，也不阻塞单卡 P9-C。
- [x] 二次代码质量门禁清除生产 runtime assert、业务复杂函数和 `PLR2004`；compile
  安全边界修复后 nonzero-offset GPU regression 与 batch1 真实 8B exact，batch2 分叉
  如实保留，未将 rejected candidate 提升为 supported backend。

P9-B 的正式命令、测试数量、Graph metadata 与未验证边界记录在
`VERIFICATION.md` 的 P9-B 小节。P9-C 从 scaled FP8 scale lifecycle 开始，不回头把
unit-scale direct cast 改名为“已支持 FP8”。

### 13.2 P9-C 当前出口与剩余 Gate A 边界

- [x] `scaled_fp8_kv` 使用 per-token/per-KV-head K/V FP32 scales；payload 与 scales
  在 store、paged decode、COW、swap、compaction 和 CUDA Graph replay 中同生命周期。
- [x] clean `5ada892` 的 DocVQA、MuirBench、MVBench development/final 六项
  Prism BF16→scaled-FP8 formal non-inferiority 均 PASS；allocated KV pool 为 BF16
  的 `0.515625x`，节省 `48.4375%`。
- [x] clean `3ec90a5` 的 vLLM 0.24.0 formal external quality matrix 完成；同容量
  per-token-head FP8 在 DocVQA/MuirBench PASS、MVBench development/final FAIL，
  所以整体结论必须写为 MIXED。
- [ ] 完整 Gate A 仍待跨框架统一 page-table、allocator metadata 和 total physical
  bytes。当前 artifacts 明确记录 `full_physical_comparable=false`，不能只凭 allocated
  KV pool 宣称物理显存 Pareto 支配。
- [ ] scaled FP8 的正式 runtime speedup、细页 allocator，以及
  `visual_compact_scaled_fp8` 组合质量仍需独立门禁。

详细协议、逐数据集指标、CI 和 artifact SHA256 见 `VERIFICATION.md` P9-C.1/P9-C.2。
