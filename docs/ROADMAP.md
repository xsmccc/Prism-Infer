...
1. 单卡推理吞吐与能力支持：先建立 online arrival/queueing、continuous batching 和调度压力的可复现 baseline，再依据 profile 优化 scheduler、batch formation 与 decode execution；不得用 replicated-request offline throughput 代替 online serving。
2. 架构与代码质量：在现有 `238 passed, 6 skipped` 回归保护下审计 Config/全局 context、engine/runner/scheduler 边界、类型契约、错误处理和重复默认值，按小提交重构，不把大规模重写与性能优化混在一起。
...
5. P7 交付：重写 README、固定最小复现命令并整理 Known Issues。
