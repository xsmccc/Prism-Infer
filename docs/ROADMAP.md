...
| 系统性能优化 | P6.12-A runtime attention pruning 已完成，dataset-level quality 仍未通过 | P6.11 compressed KV CUDA Graph 已有 clean matrix；P6.12 新增最后-query attention runtime score、跨层/TP 聚合和 physical compaction/Graph wiring。keep=0.5 时 COCO stable prefix `3 -> 21`，但 multi-image 仅 `6 -> 7`、video 仍 `14`，所以 quality 继续 FAIL；两卡动态 TP 和 RTX hardware counter 仍待外部平台。 |
...
- [ ] P6.12 Content-aware visual pruning:
  ...
  - [ ] P6.12-B dataset-level quality：扩大固定真实样本和任务指标，调查 per-span budget、视觉网格 coverage、跨 query/layer 聚合；当前不得把 attention strategy 标记为质量 PASS。
...
- P6.12-A full regression 为 `212 passed, 6 skipped in 299.59s`；raw log 为 `data/p6_system/p612_full_regression_20260714.txt`。
...
1. 单卡研究主线 P6.12-B：在 runtime attention baseline 上加入 per-span budget/视觉网格 coverage 候选，并扩展固定真实数据集和任务级质量指标；任何候选先做 quality preflight，再进入性能实现。
