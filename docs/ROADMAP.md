...
| 系统性能优化 | P6.12-B 进行中，dataset-level quality 仍未通过 | P6.11 compressed KV CUDA Graph 已有 clean matrix；P6.12-A 已完成 runtime attention scorer。P6.12-B 发现 global top-k 在双 span 中分配为 `124/72`、`109/87`，但等额 `98/98` per-span quota 未改善 stable prefix，候选策略已删除。quality 继续 FAIL；两卡动态 TP 和 RTX hardware counter 仍待外部平台。 |
...
- [ ] P6.12 Content-aware visual pruning:
  ...
  - [ ] P6.12-B dataset-level quality：扩大固定真实样本和任务指标，调查视觉网格 coverage、跨 query/layer 聚合和非等额动态预算；当前不得把 attention strategy 标记为质量 PASS。
...
1. 单卡研究主线 P6.12-B：等额 per-span budget 已被证明无改善；下一候选转向视觉网格 coverage、跨 query/layer 聚合或数据驱动的动态 span budget，同时扩展固定真实数据集和任务级质量指标；任何候选先做 quality preflight，再进入性能实现。
