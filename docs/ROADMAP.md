...
| 秋招旗舰化 | P9-A PASS；下一阶段 P9-B | RFC、workload/quality manifest、20-cell clean page matrix、page16/256 NCU raw evidence，以及最终回归/文档门禁均已完成。 |
...
1. 冻结并提交 P9-B config/global-state/backend contract，不再把架构迁移混入量化 diff。
2. 启动 P9-C scaled FP8，并依次通过 component、model-precision logits/PPL、标准质量和 E2E；
   未通过前不做 page-head scale 性能变体。
3. 用 clean NCU/NSYS 选择 full-step Graph 与 split-GQA/context kernel，不为学习 DSL
   强行替换没有瓶颈证据的 Triton 路径。
4. 最后接 thin server 与 H3 external SLO，2026-08-06 冻结工程，转入复习。
