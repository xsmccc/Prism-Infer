...
- [x] P9-C.3 Prism full-physical Gate A：冻结 scaled-FP8 的 KV pool、torch peak、
  current-process NVML、capacity 与质量/延迟 Pareto；跨框架统一 allocator metadata
  字节合同不进入本轮 headline。
...
- unit-scale `fp8_kv` 继续作为 rejected baseline；`scaled_fp8_kv` 已携带独立 K/V
  scales，并在三数据集 development/final 六项 formal gate 全 PASS。相同 capacity 下
  KV pool 为 BF16 的 `0.515625x`，current-process NVML 少 `1972 MiB`；相同约
  `4 GiB` KV 预算下 capacity 从 `28,928` 增至 `56,320` tokens（`+94.69%`）。
...
- clean `4779342` 的最终 H1/H2 matrix 已把媒体、prompt-token hash、GPU UUID、
  warmup/repeat 和 greedy 稳定性对齐。Prism BF16 TPOT 相对 SGLang 低
  `4.54%–4.83%`、相对 vLLM 低 `6.13%–6.27%`；scaled-FP8 在约 `4 GiB` KV 预算下
  相对 SGLang 低 `1.06%–1.12%`、相对 vLLM 低 `2.55%–2.77%`。E2E 并非所有格都领先，
