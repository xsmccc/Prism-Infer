...
1. P6.0: 针对 `fp8_kv` 和 `visual_prune` 的慢路径做 kernel/graph 优化；当前 P5 只证明显存/KV bytes 收益，不证明吞吐收益。
2. P6.1: 建立与 vLLM/SGLang 对比前的 benchmark 协议，记录版本/commit、启动参数、输入集合、warmup/repeat、显存限制和采样配置。
3. P7: 更新 README 和技术报告，把 P5 的收益边界写清楚。
