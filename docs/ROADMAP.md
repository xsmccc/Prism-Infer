...
| 系统性能优化 | P6.12-B 进行中，dataset-level task quality 仍未通过 | P6.12-A 已完成 runtime attention scorer；等额 per-span quota 已被拒绝。P6.12-B 新增 7 张固定 COCO 图片和 dataset fidelity 汇总：keep=0.5 时 attention/uniform 的 exact request rate 为 `3/7` 与 `0/7`，micro prefix ratio 为 `0.696/0.304`，physical token ratio 同为 `0.535x`。该结果是 dirty quality preflight，不是 task accuracy；两卡动态 TP 和 RTX hardware counter 仍待外部平台。 |
...
- P2 阶段外能力已在 P3 补齐多图、视频、mixed batch、长输出稳定前缀、logits/ppl 分布、VL CUDA Graph decode 和 paged decode kernel；仍未支持 prefix-cache/chunked-prefill VL mixed batch，P3 kernel 也只是 baseline kernel。
...
- [ ] P6.12 Content-aware visual pruning:
  ...
  - [x] dataset fidelity 工具：新增 `pruning_fidelity.py` 和 CLI，强制同 manifest/case/model/execution、schema-v4 physical KV、candidate 全 case coverage 和 span-audit 一致性；输出 exact rate、prefix distribution、physical/bytes ratio。
  - [x] 固定真实集从 1 张 COCO 扩展到 7 张，按 `4+3` 组成两个 mixed-request batch；官方 URL、SHA256、尺寸和下载后 materialization 均有测试。
  ...
  - [ ] P6.12-B dataset-level task quality：当前只有相对 off baseline 的 greedy fidelity；还需要带可审计 reference answer/caption 的任务指标，再调查视觉网格 coverage、跨 query/layer 聚合和动态预算。当前不得把 attention strategy 标记为质量 PASS。
...
- P6.12-B dataset fidelity harness 已完成：7 张固定 COCO 图片、2 个 batch case、schema-v4 汇总 CLI、`65 passed` focused guards 和 `82 passed` 受影响回归。主 runner 新增可审计的 `--disable-prefix-caching`，用于避免 mixed-VL quality batch 进入尚未支持的 prefix-hit prefill。
- 7-image keep=0.5 dirty validation 中，attention 相比 uniform 将 exact request rate 从 `0/7` 提高到 `3/7`，micro prefix ratio 从 `0.304` 提高到 `0.696`；两者 physical token/active-byte ratio 均为 `0.535x/0.538x`。最差 attention prefix 仍只有 `7/32`，所以不标记 quality PASS。
...
- P6 当前单卡 execution/compression 主线与 TP variable-size control plane 已完成，但阶段出口不是无条件 PASS：P6.6 quality FAIL，P6.8 两卡动态矩阵 BLOCKED，P6.2-B/clean formal rerun待外部平台。下一单卡研究主线应进入内容感知 pruning 与质量评估，或进入 P7 整理交付；不得先写“全面超过外部框架”。
...
1. 单卡研究主线 P6.12-B：保留当前 7-image fidelity suite 作为固定回归，补充带 reference answer/caption 的任务集；候选转向跨 query/layer 聚合或视觉网格 coverage。任何新策略先同时通过 fidelity 和 task metric preflight，再进入性能实现。
2. P6.8：在两卡平台显式运行 `tests/test_llm_vl_tp2.py`，再补 logits、NCCL、per-GPU memory 与 latency 矩阵。
3. P6.2-B：在开放 hardware counter 的 RTX PRO 6000 或等价平台补 NCU/SM utilization，保持 workload/commit/config 可审计。
4. P7 交付：在质量路线形成首个可用策略后，重写 README、固定最小复现命令并整理 Known Issues。
