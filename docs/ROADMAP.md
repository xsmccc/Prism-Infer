...
| 系统性能优化 | P6.11 compressed KV CUDA Graph 已完成，阶段仍有明确未通过项 | `visual_compact`、`fp8_kv`、`visual_compact_fp8` 已接入 CUDA Graph decode，同压缩模式 eager/Graph output32 token exact，batch1 decode speedup `1.84x-1.86x`；组合模式 batch1-8 speedup `1.74x-1.88x`。uniform quality 仍 FAIL、两卡动态 TP 被当前单卡硬件阻断；RTX hardware counter 和 clean-commit formal rerun仍待外部条件。 |
...
- [x] P6.10 可执行阶段 Review:
  ...
  - [ ] clean-commit formal performance rerun：当前 P6 工作区尚未提交，所有性能记录仍为 `git_dirty=true`。
- [x] P6.11 Compressed KV CUDA Graph:
  ...
  - single-image output32、warmup/repeat `2/5`：compact、FP8、combo decode speedup 分别为 `1.8489x/1.8406x/1.8570x`，每组 eager/Graph token SHA256、physical tokens 和 active bytes exact。
  - combo batch `1/2/4/8`、output32：decode speedup `1.8418x/1.8785x/1.8169x/1.7411x`，Graph decode throughput `57.14/112.71/218.57/417.61 tok/s`；这是 replicated-request offline decode，不是 online serving 结果。
  ...
  - 当前 P6.11 性能证据仍为 commit `ac6e01d` 的 `git_dirty=true` validation；不改变 P6.6 uniform pruning quality FAIL，也不构成超过 vLLM/SGLang 的新对比。
...
- P6.11 已完成 physical compression CUDA Graph：同压缩模式 output32 eager/Graph token exact，batch1 decode speedup `1.84x-1.86x`；combo batch1-8 speedup `1.74x-1.88x`，output128 token exact。logical `visual_prune` 继续显式限制为 eager。
- P6.11 post-change 全量回归为 `208 passed, 6 skipped in 267.79s`；raw log 为 `data/p6_system/p611_full_regression_20260714.txt`。
- P6.2 尚未无条件 PASS：RTX 5090 SM utilization 没有成功采集，所有 profile 仍为 dirty-worktree 定位实验。
...
1. 单卡研究主线：用 P4/P5 trace 建立内容感知 visual-token ranking，替代 uniform pruning，并用固定真实数据集评估任务质量、stable prefix、显存和 Graph TPOT。
2. clean-commit formal rerun：提交 P6.11 后复跑三组 batch1 eager/Graph 和 combo batch1-8 关键矩阵，使性能证据从 validation 升级为可发布记录。
3. P6.8：在两卡平台显式运行 `tests/test_llm_vl_tp2.py`，再补 logits、NCCL、per-GPU memory 与 latency 矩阵。
4. P6.2-B：在开放 hardware counter 的 RTX PRO 6000 或等价平台补 NCU/SM utilization，保持 workload/commit/config 可审计。
