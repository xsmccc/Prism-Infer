...
| 系统性能优化 | P6.11 compressed KV CUDA Graph 已完成并有 clean-commit formal matrix，阶段仍有明确未通过项 | `visual_compact`、`fp8_kv`、`visual_compact_fp8` 已接入 CUDA Graph decode，同压缩模式 eager/Graph output32 token exact，clean batch1 decode speedup `1.84x-1.85x`；组合模式 clean batch1-8 speedup `1.76x-1.94x`。uniform quality 仍 FAIL、两卡动态 TP 被当前单卡硬件阻断，RTX hardware counter 仍待外部平台。 |
