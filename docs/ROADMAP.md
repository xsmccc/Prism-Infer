...
| 系统性能优化 | P6.10 可执行 Review 已完成，阶段有明确未通过项 | 已完成 physical compaction、FP8 paged attention、组合 Pareto、32GB capacity、vLLM/SGLang external baseline、variable-size TP IPC 和 post-IPC full regression。组合容量为 off 的 `4.016x`，但 uniform quality FAIL、Prism eager TPOT 约慢外部 `2x`、两卡动态 TP 被当前单卡硬件阻断；RTX hardware counter 和 clean-commit formal rerun仍待外部条件。 |
...
- P6 当前单卡可执行工作与 TP variable-size control plane 已完成，但阶段出口不是无条件 PASS：P6.6 quality FAIL，P6.8 两卡动态矩阵 BLOCKED，P6.2-B/clean formal rerun待外部平台。下一单卡工程任务应让 compressed layout 接入 CUDA Graph，或进入 P7 整理交付；不得先写“全面超过外部框架”。
...
1. 单卡性能主线：设计 compressed layout 的 graph-safe metadata/control path，并以 P6.3 off/CUDA Graph 矩阵为固定 baseline。
...
4. clean-commit formal rerun：提交当前工作区后复跑关键 correctness/performance 矩阵。
