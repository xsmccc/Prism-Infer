...
- P9-A 已 PASS；保留 `BatchPlan`、`ModelExecutor`、`SchedulerPolicy`，P9-B 只做配置、
  全局 state 与 backend contract 的原子迁移，不做无意义重写。
...
1. 先提交 P9-B config/global-state/backend contract，再启动任何量化或 kernel diff。
2. scaled FP8 依次通过 component、model-precision logits/PPL、标准质量和 E2E；
