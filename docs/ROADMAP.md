...
| 系统性能优化 | P6.12-B reference task gate 已完成，首个合格策略仍未找到 | P6.12-A 已完成 runtime attention scorer；等额 per-span quota 已被拒绝。P6.12-B 现有 7 张固定 COCO 图片、35 条 caption reference、schema-v5 decoded-text 证据和 dataset task gate。keep=0.5 时 attention 的 token-F1 drop 为 `0.006351`，但 ROUGE-L drop 为 `0.012413 > 0.01`，因此整体仍 FAIL；uniform 的 ROUGE-L drop 为 `0.036365`。这些是 commit `9e5db53`、`git_dirty=true` quality preflight，不是标准 COCO accuracy 或性能 claim；两卡动态 TP 和 RTX hardware counter 仍待外部平台。 |
...
- [ ] P6.12 Content-aware visual pruning:
  ...
  - [ ] P6.12-C 收尾：将 last1 固化为默认后，在新 clean commit 上验证默认路径、multi-image/video/mixed smoke、`warmup=2/repeat=5` 稳定性能矩阵和 full regression；完成前不发布 scorer TTFT/throughput 收益。
...
1. 单卡研究主线 P6.12-B：保留当前 7-image/35-caption fidelity 与 reference-task suite 作为固定回归；候选转向跨 query/layer 聚合、视觉网格 coverage 或动态预算。任何新策略先同时通过 greedy fidelity 与 token-F1/ROUGE-L task gate，再进入稳定性能实现。
