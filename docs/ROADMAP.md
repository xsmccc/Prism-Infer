...
- [x] P2.8 图文 full logits 与 layerwise strict 对齐: 修复 VisionEncoder RoPE buffer 与 PatchMerger eps，补充 `tests/test_full_model_vl.py`、现位于 `tools/debug/full_model_vl_layerwise.py` 的手工诊断和 `tests/test_vision_rope_init.py`。
...
- P6 冻结时单卡 execution 与 BF16 visual-compaction/content-aware quality 主线已完成，
  TP variable-size control plane 也已落地；当时未解决的 unit-scale FP8 与 counter 缺口
  后续分别由 P9-C scaled FP8 和 P9-A NCU 闭环。P6.8 两卡动态矩阵仍因资源边界未验证。
...
- [x] 硬件资源边界已纠正：仅 GPU0 属当前租约；宿主机额外可见设备不作为可用性或
  NCCL 证据，TP2 标记 NOT RUN / UNVERIFIED。
- [x] 冻结 H1/H2/H3、标准质量集与 canonical hash/revision manifest；媒体物化 hash
  在首次标准质量运行前生成。
- [x] 完成结构化 paged-decode benchmark 的 focused test 与 clean formal matrix。
- [x] 保存 page16/256 NCU raw report，并同步 Known Issues/Verification。
- [x] 最终 focused regression `64 passed in 6.99s`；`compileall`、57 个本地 Markdown
  链接、artifact/hash、`git diff --check` 与 GPU release gate PASS。

### P9-B：架构硬化

- [x] 建立六个 typed config domain、strict flat compatibility adapter 与 frozen
  HF-resolved runtime Config；runtime EOS/KV capacity 使用 replacement，不原地修改。
- [x] 未知字段、隐式 bool coercion、非法 page/backend/quantization/serving 组合在模型
  加载和 CUDA 初始化前 fail closed。
- [x] 删除 `Sequence` 类级 page/ID state；page 与 request ID 显式传递，engine allocator
  可注入，TP pickle 保留 request state 并拒绝 legacy tuple。
- [x] 建立 immutable `DeviceModelInputs/DeviceBatch` 与
  `ExecutionBackend.prepare/warmup/capture/execute/release` contract；executor 只调用
  `run_plan`，不把 mutable Sequence 带入执行边界。
- [x] 修复同进程重复创建 8B engine 的 runner/backend ownership leak；显式退出与
  runner-exit-failure 均确定性断开模型/KV 引用，三组 VL HF -> Prism 连续门禁 PASS。
- [x] 单卡 Qwen3-VL-8B eager 与 CUDA Graph capture/replay smoke PASS，Config spawn
  PASS，GPU0 release PASS；TP2 因当前未分配双卡资源而 NOT RUN / UNVERIFIED。
- [x] P9-B 未混入 scaled FP8、NVFP4、W/A quantization 或 kernel 候选。
- [x] 2026-07-19 二次架构复审完成：整个 `prism_infer`（含 analysis/evidence 路径）的
  `C901/PLR0911/PLR0912/PLR0915`、生产 runtime assert 和 `PLR2004` 全部清零并加入
  CPU CI；scheduler/engine/online 与 benchmark/quality validator 职责拆分后 CPU
  `368 passed`、GPU0 `18 passed`。compile 只捕获纯 QKV/QK-Norm/M-RoPE，mutable KV
  store/paged decode 留在 validated runtime；batch2 长输出仍分叉，因此 compile 状态
  继续为 REJECTED。
- [x] 2026-07-20 修复 vision backend 隐式选择：`MultimodalConfig` 显式固定
  `sdpa/flash_attn`，默认 SDPA，非法/缺失 capability fail closed；单图、多图、视频
  full logits bit-exact，VL CUDA Graph mixed batch token exact。FlashAttention 保留为
  P9-D 独立质量/性能候选，不再由包存在性或 segment shape 自动启用。
- [x] 2026-07-20 建立 P9 fresh-process 性能编排：parent 不导入 torch，每个
  mode/repeat 独立 child，按物理 UUID 执行 idle/release gate，使用 ABBA/BAAB、严格
  comparability、process-level bootstrap 95% CI 和机器可读 formal eligibility；dirty
  两进程 smoke 15/15 checks PASS，正式 H1 matrix 待 clean checkpoint 后运行。
- [x] 修复 H1 batch4 benchmark 的静态 batch 假设：按 sequence 完成边界保存 prompt KV，
  schema-v9 记录 actual decode histogram 与 actual→captured Graph bucket；BF16/scaled-FP8
  dirty smoke 均观察到 `1/2/3` 动态 batch、Graph `1→1/2→2/3→4`，token exact 且进程
  前后显存归零。该结果只关闭机制门禁，不作为性能 claim。
- [x] 将四个 script-only full-model logits 文件变成 pytest 可收集的 slow model gates，
  并统一 direct-script 非 PASS 退出码；修复后纯文本、单图、多图、视频 4 项均与 HF
  bit-exact，避免组合 pytest 被其他 item 掩盖成假 PASS。

### P9-C–P9-F 执行顺序

- [x] P9-B 架构硬化：typed domain config、未知参数拒绝、删除 `Sequence` 全局 page/ID
  state、immutable `DeviceBatch` 和显式 execution backend contract。
- [x] P9-C.1 KV/量化 correctness：per-token/per-KV-head scaled FP8、scale
  全生命周期和标准质量 gate；Prism 内部六个 formal cell 全 PASS。
- [x] P9-C.2 external quality：vLLM BF16/per-token-head FP8 development/final matrix
  完成，结果为 MIXED；semantic input exact。
- [ ] P9-C.3 full-physical Gate A：补齐跨框架 page-table/allocator metadata 统一字节
  合同、细页 allocator 与 Pareto 判定；W/A shootout 只有在不挤占 P9-D 时启动。
- [ ] P9-D Compiler/Graph/Kernel：按 `docs/P9_COMPILER_GRAPH_PLAYBOOK.md` 的完整
  pipeline 合同完成 greedy full-step Graph、compile+Graph 候选、split-GQA/context
  paged attention 和 NCU/NSYS 闭环；成功与 rejected 问题都进入 Issue Log。
...
- P9-A/P9-B 已 PASS；`BatchPlan`、`ModelExecutor`、`SchedulerPolicy` 保留，配置、
  request/page identity 与 backend contract 已完成原子迁移。
- unit-scale `fp8_kv` 继续作为 rejected baseline；`scaled_fp8_kv` 已携带独立 K/V
  scales，并在 clean `5ada892` 的三数据集 development/final 六项 formal gate 全 PASS。
  allocated KV pool 为 BF16 的 `0.515625x`，但尚不包含跨框架统一的
  page-table/Python allocator 字节。
- clean `3ec90a5` 的 vLLM per-token-head FP8 external quality 为 DocVQA/MuirBench
  PASS、MVBench development/final FAIL。该结果是预注册稳定性门禁 MIXED，不是
  “Prism accuracy 显著领先”；vLLM MVBench accuracy 点估计实际更高。
- clean `29c0dbe` BF16 page matrix 为 20/20 correctness PASS；在预注册的
  batch `1/8`、context `4096/8192` 四格中，最优 page16/32 kernel median 相对
  page256 低 `13.6%–20.1%`。这只是 paged-decode microbenchmark，不是 full-engine
  TPOT，且当前整除 context 未覆盖页尾碎片，不能据此直接修改默认 page size。
- 新 NCU 显示 batch8/context4096 的 achieved occupancy 约 `12.5%`、waves/SM
  `0.17–0.19`；page16 duration `449.95 us`，page256 `543.26 us`。简单把四个 GQA
...
1. 完成 P9-C.3 的 full-physical 字节合同，明确 Gate A 是 PASS、FAIL 还是不可比；
   不重复运行已经冻结的质量矩阵。
2. 用 clean NCU/NSYS 进入 P9-D，选择 full-step Graph 与 split-GQA/context kernel，不为学习 DSL
   强行替换没有瓶颈证据的 Triton 路径。
3. 最后接 thin server 与 H3 external SLO，2026-08-06 冻结工程，转入复习。
