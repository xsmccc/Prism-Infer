...
- 状态: `INVESTIGATING`（logits、Systems分类与P7.5 packed MLP已闭环；NCU counter待外部平台）
...
6. ~~P7.5评估占 `70.55%` 的 linear/GEMV。~~ gate/up packing已让每步少36个linear，
   8个clean cell的decode TPOT改善`0.483%–0.762%`。后续只在counter权限可用时对
   top kernel或合计约`15.15%`的elementwise/copy/reduction继续优化。
...
- 双方同条件的 Nsight Systems node capture、Graph 外 logits、Prism replay逐类
  归因与P7.5 gate/up实现已完成；但 top kernel 的 NCU counter尚未完成。`70.55%`是当前
...
model-precision logits后，五类 clean best-stable workload的 compact Prism/vLLM
...
clean `021d4e2` node trace确认packed gate/up把linear/GEMV从`253`降至`217`、总
kernels从`2,000`降至`1,964`，kernel busy从`12.815 ms`降至`12.721 ms`。八个
unprofiled offline cells全部token exact，decode TPOT改善`0.483%–0.762%`。这只解释
外部差距中的极小一部分；更新后的vLLM对比尚未因该不足1%的变化重跑，也不能据此
改写既有`1.34x–1.40x`边界。
