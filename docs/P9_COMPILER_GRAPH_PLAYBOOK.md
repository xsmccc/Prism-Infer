...
`128.417 -> 126.123 ms`（`-1.79%`），CUDA event region
`137.077 -> 135.228 ms`；完整 prefill kernel busy
`177.820 -> 175.801 ms`。36 个原生 SiLU 与 36 个对应 multiply 合并为 36 个
`_fused_swiglu_kernel`，同步与 memcpy 数不变。H1/H2 128-token 全流程分别保持原
SHA256 `76ad1f...14c6` 与 `4a61f1...166f`。H2 TTFT 仍有已知双峰，因此不使用
repeat7 中位数形成 E2E 收益 claim；本项只报告可重复的激活、language-model region
和完整 prefill kernel 收益。
...
| model-only CUDA Graph | 历史内部基线 | supported | 四个 formal cell 与 NSYS 归因已完成 |
| greedy full-step CUDA Graph | decoder + LM head + guarded selection | supported | 作为最终 compile+Graph 路径的 capture 外层 |
| unsafe pure/full-decode compile | 历史 whole-decode 候选 | rejected | OOM、alias 与 batch2 分叉证据保留，不是当前受控 compiled region |
| compile + full-step Graph | batch1无状态热点 + 完整Graph replay | supported | clean `4779342`：BF16 H1/H2 TPOT 相对 SGLang 低 `4.54%–4.83%`、相对 vLLM 低 `6.13%–6.27%`；scaled 同约 4 GiB budget 仍领先 |
...
2. **已完成**：用 NSYS 分解 prefill、完整 decode step、Graph 外 CPU/GPU 工作和同步。
3. **已完成**：model-precision/guarded LM-head 路径使用稳定 device buffer，不做逐 token
   整权重 FP32 转换。
4. **已完成**：greedy candidate/rerank 与必要状态纳入完整 decode Graph，结果只做最小 D2H。
5. **已完成**：验证 full-step bucket、padding、KV/scale view、退出和重复 engine lifecycle。
6. **已完成**：scaled-FP8 KV 接入同一 compile+Graph boundary，记录 compile cold/capture 元数据。
7. **已止损**：profile 指向 weight GEMV 后仍验证 GQA4 merge 与 split-K；两者在 H1/H2
   慢约 `1.85x–1.90x`，候选代码撤销。
8. **已完成**：clean `4779342` 冻结 H1/H2 × Prism/vLLM/SGLang 的同 prompt、
   warmup2/repeat5 full-engine 结果。
...
截至 2026-07-23，P10 已在 full-step compile/Graph 路径完成两条最终 profile：

- BF16 latency profile：H1/H2 TPOT `9.8821/9.8680 ms`；
- scaled-FP8 capacity profile：约 4 GiB KV budget 下 capacity `56,320 tokens`，
  H1/H2 TPOT `10.2363/10.2588 ms`；
- 同提交 SGLang 为 `10.3520/10.3689 ms`，vLLM 为 `10.5276/10.5278 ms`；
- 八个单元同 commit/GPU/prompt hash，warmup2/repeat5，跨 repeat 输出稳定。

完整结论与禁止外推项见`docs/P10_FINAL_RESULTS.md`。P9 的 fresh-process eager/Graph
CI 继续作为内部 backend 收益证据；P10 的外部排名只使用无 profiler 的同提交
warmup/repeat 冻结集，两者不混算。
