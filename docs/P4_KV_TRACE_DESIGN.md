...
- 能输出 visual attention mass、attention entropy、token importance、层间冗余和 head 差异的离线统计。
...
- `record_attention_layer(...)`: 记录每层 Q/K/V、span KV norm、attention mass、attention entropy 和 top token。
- `summarize_trace(...)`: 离线汇总 visual/text KV norm、attention mass、attention entropy、head 差异和层间相似性。
...
- `attention_entropy`: 当前 query attention probability 的 Shannon entropy；归一化 entropy 用于衡量注意力分布是否集中。
- `visual_attention_entropy`: 在 image/video token 内部归一化后的条件 entropy，用于判断视觉注意力是否集中到少量 token。
