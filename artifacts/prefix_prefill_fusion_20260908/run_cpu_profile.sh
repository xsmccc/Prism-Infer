#!/usr/bin/env bash
#SBATCH --job-name=prism-cpu-prefill
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gpus=1
...
for SOURCE in cooperative prefill-fused; do
    export PYTHONPATH=$STUDY_ROOT/$SOURCE
    python "$STUDY_ROOT/profile_cpu.py" --config "$STUDY_ROOT/engine.json" \
        --output "$STUDY_ROOT/runs/cpu-$SOURCE-$SLURM_JOB_ID.json"
done
