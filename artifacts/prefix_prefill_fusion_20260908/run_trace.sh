#!/usr/bin/env bash
#SBATCH --job-name=prism-fusion-trace
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gpus=1
...
PID_FILE=$STUDY_ROOT/runs/fusion-server-$SLURM_JOB_ID.pid
SERVER_PID=
PROFILE_PID=
cleanup() {
...
nsys profile --trace=cuda,nvtx,osrt --sample=none --cpuctxsw=none \
    ...
    --output="$STUDY_ROOT/runs/fusion-$SLURM_JOB_ID" \
    ...
    --output "$STUDY_ROOT/runs/server-fusion-trace-$SLURM_JOB_ID.json" &
PROFILE_PID=$!
...
python "$STUDY_ROOT/bench_serving_preprocessing.py" \
    --base-url "http://127.0.0.1:$PORT" --label fusion-trace --repeat 1 \
    --output "$STUDY_ROOT/runs/http-fusion-trace-$SLURM_JOB_ID.json"
