#!/usr/bin/env bash
#SBATCH --job-name=prism-prefill-trace
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gpus=1
...
PID_FILE=$STUDY_ROOT/runs/prefill-server-$SLURM_JOB_ID.pid
SERVER_PID=
PROFILE_PID=
cleanup() {
...
nsys profile --trace=cuda,nvtx,osrt --sample=none --cpuctxsw=none \
    ...
    --output="$STUDY_ROOT/runs/prefill-$SLURM_JOB_ID" \
    python "$STUDY_ROOT/serve_preprocessing_study.py" \
    --model /home/lcpu/87120912/models/Qwen3-VL-8B-Instruct \
    --config "$STUDY_ROOT/engine-cooperative.json" --port "$PORT" \
    --nvtx --pid-file "$PID_FILE" \
    --output "$STUDY_ROOT/runs/server-prefill-trace-$SLURM_JOB_ID.json" &
PROFILE_PID=$!
...
python "$STUDY_ROOT/bench_serving_preprocessing.py" \
    --base-url "http://127.0.0.1:$PORT" --label prefill-trace --repeat 1 \
    --output "$STUDY_ROOT/runs/http-prefill-trace-$SLURM_JOB_ID.json"
