...
def gather_paged_kv_vectorized(
    k_cache: torch.Tensor,
    v_cache: torch.Tensor,
    block_ids: torch.Tensor,
    context_len: int,
    *,
    ...
    """Vectorized paged gather + dequant for the flash-attn fast path.

    与 :func:`gather_paged_kv_for_sequence` 语义一致，但用一次 index_select +
    reshape 取代 Python 逐 block 循环与 ``.tolist()`` 同步；dequant 顺序与
    :func:`dequantize_cache_for_attention` 相同（先转 dtype 再乘 scale），
    数值行为与参考路径一致。
    """
