...
class HTTPRangeReader(io.RawIOBase):
    ...
    def _range_get(self, url: str, start: int, end: int) -> Any:
        ...
        actual_start, actual_end, total = parse_content_range(response.headers.get("Content-Range"))
    ...
    def _fetch_chunk(self, chunk_index: int) -> bytes:
        ...
        if len(payload) != expected_length:
            raise OSError(
                f"range payload length mismatch: expected={expected_length}, actual={len(payload)}"
            )
