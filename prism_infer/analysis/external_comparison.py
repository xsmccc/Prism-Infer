...
def validate_external_record(record: Mapping[str, Any]) -> None:
    ...
    if schema_version not in SUPPORTED_EXTERNAL_SCHEMA_VERSIONS:
        raise ValueError(
            f"external record schema_version must be one of {SUPPORTED_EXTERNAL_SCHEMA_VERSIONS}"
        )
    ...
    _validate_external_sections(record)
    _validate_external_environment(record["environment"])
    _validate_external_measurement(record["measurement"])
    _validate_external_correctness(record["correctness"])
    _validate_external_metric_evidence(record)
    if schema_version >= EXTERNAL_PROTOCOL_SCHEMA_VERSION:
        _validate_external_protocol(record)


def _validate_external_sections(record: Mapping[str, Any]) -> None:
    for section in (
        "environment",
        "model",
        "backend",
...
def _validate_external_environment(environment: Mapping[str, Any]) -> None:
...
def _validate_external_measurement(measurement: Mapping[str, Any]) -> None:
...
def _validate_external_correctness(correctness: Mapping[str, Any]) -> None:
...
def _validate_external_metric_evidence(record: Mapping[str, Any]) -> None:
    for path in (
...
def _validate_external_protocol(record: Mapping[str, Any]) -> None:
    ...
    _validate_external_protocol_fields(protocol)
    _validate_external_v2_environment(record["environment"])
    _validate_external_backend(record["backend"])
    _validate_external_sampling(record)


def _validate_external_protocol_fields(protocol: Mapping[str, Any]) -> None:
...
def _validate_external_v2_environment(environment: Mapping[str, Any]) -> None:
    for key in ("gpu_uuid", "driver", "compute_capability"):
        if not isinstance(environment.get(key), str) or not environment[key]:
            raise ValueError(f"external environment.{key} must be non-empty")


def _validate_external_backend(backend: Mapping[str, Any]) -> None:
...
def _validate_external_sampling(record: Mapping[str, Any]) -> None:
...
def _p7_comparability_checks(
    ...
    *,
    ...
    return {
        ...
        "prompt_tokens": (
            prism_workload.get("prompt_tokens") == external_workload.get("prompt_tokens")
        ),
        "model_config": (
            prism_model.get("config_sha256") == external_model.get("config_sha256")
            and prism_model.get("config_sha256") not in (None, "unknown")
        ),
        "dtype": prism_model.get("dtype") == external_model.get("dtype"),
        "tensor_parallel_size": (
            prism_model.get("tensor_parallel_size") == external_model.get("tensor_parallel_size")
        ),
        "max_model_len": (prism_model.get("max_model_len") == external_model.get("max_model_len")),
        "max_num_batched_tokens": (
            prism_model.get("max_num_batched_tokens")
            == external_model.get("max_num_batched_tokens")
        ),
        "max_num_seqs": (prism_model.get("max_num_seqs") == external_model.get("max_num_seqs")),
        "block_size": (
            prism_model.get("kvcache_block_size") == external["backend"].get("block_size")
        ),
        "kv_pool_bytes": (
            prism["kv_cache"].get("bytes") == external_model.get("kv_cache_memory_bytes")
        ),
        "prefix_caching": (
        ...
        "execution_profile": _execution_profile_matches(prism, external, profile),
        "same_gpu": (
            prism_environment.get("gpu_uuid") == external_environment.get("gpu_uuid")
        ...
        "clean_harness": (
            ...
            and protocol.get("harness_git_commit") == prism_environment.get("git_commit")
        ),
...
def compare_external_records(
    ...
    *,
    ...
    baselines = _build_prism_external_baselines(
        prism_records,
        prism_mode=prism_mode,
        prism_keep_ratio=prism_keep_ratio,
    )

    rows: list[dict[str, Any]] = []
    for external in external_records:
        validate_external_record(external)
        external_profile = external.get("protocol", {}).get("comparison_profile", "legacy_p6")
        ...
        rows.append(
            _build_external_comparison_row(
                baselines[key],
                external,
                key=key,
    ...
    if comparison_profile is not None and not rows:
        raise ValueError(f"no external records matched comparison_profile={comparison_profile!r}")
    return rows


def _build_prism_external_baselines(
    prism_records: Sequence[Mapping[str, Any]],
    *,
    prism_mode: str,
    prism_keep_ratio: float,
...
def _comparison_metrics(
    ...
    return {
        "prism_tpot": prism_tpot,
        "external_tpot": external["timing_ms"]["decode_tpot"]["median"],
        "prism_throughput": prism_throughput,
        "external_throughput": external["throughput"]["e2e_output_tokens_per_s"]["median"],
        "prism_peak": prism_peak,
        "external_peak": external_memory.get("peak_allocated", external_memory.get("process_used"))[
            "median"
        ],
        "prism_memory_measurement": prism["memory_mb"].get("measurement", "torch_cuda_allocator"),
        "external_memory_measurement": external_memory.get("measurement", "torch_cuda_allocator"),
    }
...
def _comparison_checks(
    ...
    input_token_exact = prism["workload"]["prompt_tokens"] == external["workload"]["prompt_tokens"]
    checks = (
        _p7_comparability_checks(prism, external, profile=external_profile)
        if external["schema_version"] >= EXTERNAL_PROTOCOL_SCHEMA_VERSION
        else {"prompt_tokens": input_token_exact}
    )
    reasons = [name for name, passed in checks.items() if not passed]
    return input_token_exact, checks, reasons


def _build_external_comparison_row(
    prism: Mapping[str, Any],
    external: Mapping[str, Any],
    *,
    key: tuple[Any, ...],
    ...
    metrics = _comparison_metrics(prism, external, key)
    input_exact, checks, non_comparable_reasons = _comparison_checks(
        prism, external, external_profile
    )
    performance_comparable = not non_comparable_reasons
    memory_comparable = (
        metrics["prism_memory_measurement"] == metrics["external_memory_measurement"]
    )
    return {
        ...
        "external_cudagraph_mode": external["backend"].get("cudagraph_mode"),
        "framework": external["environment"]["framework"],
        "framework_version": external["environment"]["framework_version"],
        "framework_source_commit": external["environment"]["framework_source_commit"],
        "stable_prefix_per_request": stable_prefix_lengths(prism_tokens, external_tokens),
        "token_exact": prism_tokens == external_tokens,
        "prism_tpot_median_ms": metrics["prism_tpot"],
        "external_tpot_median_ms": metrics["external_tpot"],
        "external_to_prism_tpot_ratio": (
            metrics["external_tpot"] / metrics["prism_tpot"] if performance_comparable else None
        ),
        "prism_e2e_output_tokens_per_s": metrics["prism_throughput"],
        "external_e2e_output_tokens_per_s": metrics["external_throughput"],
        "external_to_prism_throughput_ratio": (
            metrics["external_throughput"] / metrics["prism_throughput"]
            if performance_comparable
            else None
        ),
        "prism_peak_allocated_mib": metrics["prism_peak"],
        "external_peak_allocated_mib": metrics["external_peak"],
        "memory_comparable": memory_comparable,
        "prism_memory_measurement": metrics["prism_memory_measurement"],
        "external_memory_measurement": metrics["external_memory_measurement"],
        "external_to_prism_peak_memory_ratio": (
            metrics["external_peak"] / metrics["prism_peak"]
            if performance_comparable and memory_comparable
            else None
        ),
        "prism_physical_prompt_tokens": prism.get("kv_cache", {}).get("physical_prompt_tokens"),
        "prism_active_prompt_bytes": prism.get("kv_cache", {}).get("active_prompt_bytes"),
    }
