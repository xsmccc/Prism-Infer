...
def test_vl_chunked_prefill_requires_single_chunk():
    """P2 第一版不允许 VL 请求跨多个 prefill chunk 静默执行。"""

    _require_cuda()
    runner = _make_runner()
    runner.config.max_chunk_size = 16
    seq = _single_image_sequence()

    try:
        runner.run([seq], is_prefill=True)
    except ValueError as exc:
        assert "VL chunked prefill" in str(exc)
        print("VL chunked prefill rejection: PASS")
        return
    raise AssertionError("expected ValueError for multi-chunk VL prefill")
