...
def test_fully_cached_aligned_prompt_leaves_computable_tail() -> None:
    """Block 对齐 prompt 全命中: probe 留可计算尾巴, 分配侧仍完整复用。"""
    ...
    assert candidate == 0  # 对齐 prompt 的 probe 跳过末块 (不留零 prefill 候选)

    manager.allocate(second)
    assert second.block_table[0] == first_block  # 分配侧完整复用 (复活 cached 块)
    assert second.num_cached_tokens == 4
