"""视频 full-model logits 验证。

对比 HF Qwen3VLForConditionalGeneration 与 Prism-Infer Qwen3VLForCausalLM
在同一 synthetic video 输入上的最后 token logits。两个模型分开加载/释放，
避免同时占用 GPU。
"""
...
def run_hf_vl_forward(inputs) -> torch.Tensor:
    """运行 HF 视频 forward，返回最后 token logits。"""
...
def run_our_vl_forward(inputs, position_ids: torch.Tensor) -> torch.Tensor:
    """运行 Prism-Infer 视频 forward，返回最后 token logits。"""
...
def _run_full_model_video_verification() -> str:
    """Execute the video HF/Prism full-logits gate."""

    print("=" * 60)
    print("Prism-Infer Video VL Full Model Verification")
    ...
    print("\n[2/2] Prism-Infer video VL forward...")
...
def test_full_model_video_logits_match_hf() -> None:
    if not torch.cuda.is_available():
        pytest.skip("video full-model logits verification requires CUDA")
    assert _run_full_model_video_verification() == "PASS"


if __name__ == "__main__":
    if not torch.cuda.is_available():
        raise RuntimeError("需要 CUDA 才能运行视频 full-model logits 验证")
    if _run_full_model_video_verification() != "PASS":
        raise SystemExit(1)
